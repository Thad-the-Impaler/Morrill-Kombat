...
const portraitData = {
  'BAG': 'Assets/Portraits/BAG.png',
  'DRONE': 'Assets/Portraits/DRONE.png',
  'MANNEQUIN': 'Assets/Portraits/MANNEQUIN.png',
  'BORGUS': 'Assets/Portraits/BORGUS.png',
  'ERICTHO': 'Assets/Portraits/ERICTHO.png',
  'QUELLIC': 'Assets/Portraits/QUELLIC.png',
  'BOJDOBOJDOBOJDO': 'Assets/Portraits/BOJDOBOJDOBOJDO.png',
  'SCALENA': 'Assets/Portraits/SCALENA.png',
  'BIRDEATER': 'Assets/Portraits/BIRDEATER.png',
  'SIX IRON-NINE IRON': 'Assets/Portraits/SIX_IRON-NINE_IRON.png',
  'RELAPMI': 'Assets/Portraits/RELAPMI.png',
  'HANGMAN': 'Assets/Portraits/HANGMAN.png',
  'TUBEWARDEN': 'Assets/Portraits/TUBEWARDEN.png',
  'ORCUS': 'Assets/Portraits/ORCUS.png',
  'HEAD': 'Assets/Portraits/HEAD.png',
  'PRINTER': 'Assets/Portraits/PRINTER.png',
  'DARK BOJDO': 'Assets/Portraits/DARK_BOJDO.png',
  'SIX DRIVER-NINE IRON': 'Assets/Portraits/SIX_DRIVER-NINE_IRON.png',
  'MANEATER': 'Assets/Portraits/MANEATER.png',
  'GROOVE MCSMOOTH': 'Assets/Portraits/GROOVE_MCSMOOTH.png',
  'DARK DUPLAIRE': 'Assets/Portraits/DARK_DUPLAIRE.png',
  'CANIS': 'Assets/Portraits/CANIS.png',
  'TWINS': 'Assets/Portraits/TWINS.png',
  'THE COUNT': 'Assets/Portraits/THE_COUNT.png',
};
for (const [name, src] of Object.entries(portraitData)) {
  const img = new Image();
  img.onload = function() { portraitImages[name] = img; };
  img.src = src;
}

// --- ARNAV SPRITE ASSETS ---
const arnavImages = {};
const arnavSpriteData = {
  'ArnStationary': 'Assets/Fighters/Arnav/ArnStationary.PNG',
  'ArnPunch': 'Assets/Fighters/Arnav/ArnPunch.PNG',
  'ArnUppercut': 'Assets/Fighters/Arnav/ArnUppercut.PNG',
  'ArnLowKick': 'Assets/Fighters/Arnav/ArnLowKick.PNG',
  'ArnHighKick': 'Assets/Fighters/Arnav/ArnHighKick.PNG',
  'ArnSelect': 'Assets/Fighters/Arnav/ArnSelect.PNG',
};
for (const [name, src] of Object.entries(arnavSpriteData)) {
  const img = new Image();
  img.onload = function() { arnavImages[name] = img; };
  img.src = src;
}
// Per-sprite metadata: source dimensions plus the (x, y) pixel that should
// land on the fighter's foot/ground anchor when drawn. Foot centroids were
// computed from the alpha mask of each PNG.
const ARNAV_ANCHORS = {
  ArnStationary: { w: 933,  h: 1978, anchorX: 447, anchorY: 1726 },
  ArnPunch:      { w: 991,  h: 1978, anchorX: 401, anchorY: 1749 },
  ArnUppercut:   { w: 882,  h: 1978, anchorX: 319, anchorY: 1729 },
  ArnLowKick:    { w: 925,  h: 1978, anchorX: 127, anchorY: 1739 },
  ArnHighKick:   { w: 1578, h: 1978, anchorX: 813, anchorY: 1749 },
  // Select-card photo: bottom-anchored, no foot tracking needed.
  ArnSelect:     { w: 1582, h: 1804 },
};
// Scale factor: ~95px head-to-foot in canvas space (matches existing fighters).
const ARNAV_SCALE = 95 / (1726 - 177);

// --- THAD SPRITE ASSETS ---
const thadImages = {};
const thadSpriteData = {
  'ThadStationary': 'Assets/Fighters/Thad/ThadStationary.PNG',
  'ThadPunch': 'Assets/Fighters/Thad/ThadPunch.PNG',
  'ThadLowKick': 'Assets/Fighters/Thad/ThadLowKick.PNG',
  'ThadHighKick': 'Assets/Fighters/Thad/ThadHighKick.PNG',
  'ThadSelect': 'Assets/Fighters/Thad/ThadSelect.PNG',
  'ThadUppercut': 'Assets/Fighters/Thad/ThadUppercut.PNG',
};
for (const [name, src] of Object.entries(thadSpriteData)) {
  const img = new Image();
  img.onload = function() { thadImages[name] = img; };
  img.src = src;
}
// Per-sprite metadata: source dimensions and (anchorX, anchorY) — the
// pixel that should land on the fighter's foot/ground point. Foot anchors
// were computed from each sprite's alpha mask to keep the planted foot
// stable across poses.
const THAD_ANCHORS = {
  ThadStationary: { w: 886,  h: 1978, anchorX: 447, anchorY: 1726 },
  ThadPunch:      { w: 972,  h: 1978, anchorX: 386, anchorY: 1749 },
  ThadUppercut:   { w: 870,  h: 1978, anchorX: 268, anchorY: 1729 },
  ThadLowKick:    { w: 936,  h: 1978, anchorX: 111, anchorY: 1739 },
  ThadHighKick:   { w: 1458, h: 1978, anchorX: 817, anchorY: 1749 },
  // Select-card photo: bottom-anchored, no foot tracking needed.
  ThadSelect:     { w: 966,  h: 1375 },
};
// Scale factor: ~95px head-to-foot, matching the other fighters.
const THAD_SCALE = 95 / (1726 - 151);

// --- ZYLLEN SPRITE ASSETS ---
const zyllenImages = {};
const zyllenSpriteData = {
  'ZylStationary': 'Assets/Fighters/Zyllen/ZylStationary.PNG',
  'ZylPunch': 'Assets/Fighters/Zyllen/ZylPunch.PNG',
  'ZylUppercut': 'Assets/Fighters/Zyllen/ZylUppercut.PNG',
  'ZylLowKick': 'Assets/Fighters/Zyllen/ZylLowKick.PNG',
  'ZylHighKick': 'Assets/Fighters/Zyllen/ZylHighKick.PNG',
  'ZylSelect': 'Assets/Fighters/Zyllen/ZylSelect.PNG',
};
for (const [name, src] of Object.entries(zyllenSpriteData)) {
  const img = new Image();
  img.onload = function() { zyllenImages[name] = img; };
  img.src = src;
}
// Per-sprite metadata: source dimensions and (anchorX, anchorY) — the
// pixel that should land on the fighter's foot/ground point. Foot anchors
// were computed from each sprite's alpha mask to keep the planted foot
// stable across poses.
const ZYLLEN_ANCHORS = {
  ZylStationary: { w: 960,  h: 1978, anchorX: 447, anchorY: 1726 },
  ZylPunch:      { w: 1006, h: 1978, anchorX: 436, anchorY: 1749 },
  ZylUppercut:   { w: 894,  h: 1978, anchorX: 307, anchorY: 1729 },
  ZylLowKick:    { w: 1088, h: 1978, anchorX: 251, anchorY: 1739 },
  ZylHighKick:   { w: 1532, h: 1978, anchorX: 767, anchorY: 1749 },
  // Select-card photo: bottom-anchored, no foot tracking needed.
  ZylSelect:     { w: 1700, h: 1878 },
};
// Scale factor: ~95px head-to-foot, matching the other fighters. The
...
titleLogoImage.src = TITLE_LOGO.src;
...
for (const [name, src] of Object.entries(kingRollerSpriteData)) {
  const img = new Image();
  img.onload = function() { kingRollerImages[name] = img; };
  img.src = src;
}
// Per-sprite metadata: source dimensions and (anchorX, anchorY) — the
// pixel that should land on the assist's foot/ground point. Anchors are
// the centroid of each sprite's bottom-most opaque band (the wheels).
const KING_ROLLER_ANCHORS = {
  RollSelect: { w: 2153, h: 1513, anchorX: 937,  anchorY: 1331 },
  RollJoust:  { w: 2153, h: 1513, anchorX: 1125, anchorY: 1416 },
};
// Scale factor: ~90px tall on canvas (close to fighter height) so the
// charging backpack reads as a serious ramming threat.
const KING_ROLLER_SCALE = 90 / (1416 - 20);

// --- JAYCE ASSIST ASSETS ---
const jayceImages = {};
const jayceSpriteData = {
  'JacSelect': 'Assets/Assists/Jayce/JacSelect.PNG',
  'JacOnstage': 'Assets/Assists/Jayce/JacOnstage.PNG',
};
for (const [name, src] of Object.entries(jayceSpriteData)) {
  const img = new Image();
  img.onload = function() { jayceImages[name] = img; };
  img.src = src;
}
// Per-sprite metadata. Foot anchors are the centroid of each PNG's
// bottom-most opaque band. The muzzle offset for JacOnstage is the
// orange muzzle-tip pixel cluster, expressed as a delta from the foot
// anchor in native pixels — bullets spawn at that point.
const JAYCE_ANCHORS = {
  JacSelect:  { w: 480,  h: 1232, anchorX: 272, anchorY: 1073 },
  JacOnstage: { w: 1505, h: 1978, anchorX: 483, anchorY: 1749,
                muzzleDX: 919, muzzleDY: -943 },
};
// Scale factor: ~95px head-to-foot to match the roster fighters.
const JAYCE_SCALE = 95 / (1749 - 153);


// --- MUSIC ---
const titleMusic = new Audio('Assets/Music/titleMusic.mp3');
...
// Stage roster locked to CLASSIC + THE TEMPLE — no secret stages available.
const secretLevelOrder = [];
...
const characters = [
  {
    ...
    desc: 'The Lorekeeper',
    quote: '"We use the spicy toothpaste and everything." - Arnav',
    isArnav: true
  },
  {
    name: 'THAD',
    ...
    accent: '#3aa55a',
    outline: '#1c1c1c',
    stats: { speed: 4.7, power: 1.0, defense: 0.7 },
    desc: 'The Impaler',
    quote: '"Like a BOSS" - Thad',
    isThad: true
  },
  {
    name: 'ZYLLEN',
    ...
    accent: '#a8a8b0',
    outline: '#1c1c1c',
    stats: { speed: 4.0, power: 0.8, defense: 1.2 },
...
// Assist roster locked to PYRA + GLACIUS — no secret assists available.
const secretAssistOrder = [];
function insertAssistOrdered(assist) {
  // Assist roster locked — no-op so unlocks can't add anyone.
}
...
const attacks = {
  ...
  borgusSlam: { damage: 25, range: 70, startup: 14, active: 6, recovery: 20, hitstun: 24, blockstun: 14, height: 'high', name: 'Fist Slam', launch: true, knockbackForce: 7, shakeIntensity: 10, shakeDuration: 12 }
};
// --- RUMBLES (Fatalities) ---
...
const fightMusic = new Audio('Assets/Music/fightMusic.mp3');
...
function playFightMusic(levelName) {
  stopFightMusic();
  const music = levelMusicMap[levelName] || fightMusic;
  music.currentTime = 0;
  music.play().catch(() => {});
  currentFightMusic = music;
}

function stopFightMusic() {
  if (currentFightMusic && !currentFightMusic.paused) {
    currentFightMusic.pause();
    currentFightMusic.currentTime = 0;
  }
...
const comboSfxMap = {
  'INFERNO RUSH': sfx_infernoRush,
  'BLAZE KICK': sfx_blazeKick,
  'FLASH FREEZE': sfx_flashFreeze,
  'GLACIER SLAM': sfx_glacierSlam,
  'TOXIC BITE': sfx_toxicBite,
  'VENOM BLAST': sfx_venomBlast,
  'THUNDER CHAIN': sfx_thunderChain,
  'BOLT STRIKE': sfx_boltStrike,
  'IRON WALL': sfx_ironWall,
  'EARTHQUAKE': sfx_earthquake,
  'SHADOW STEP': sfx_shadowStep,
  'PHANTOM SLASH': sfx_phantomSlash,
};
...
function drawTitleScreen() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    ctx.fillStyle = `rgba(255,180,40,${0.1 + Math.sin(i + Date.now() * 0.003) * 0.05})`;
    ctx.beginPath();
...
function drawCharacterPreview(char, x, y, size, selected, label) {
  ...
  if (char.isZyllen) {
    ...
    if (zImg) {
      const a = ZYLLEN_ANCHORS.ZylSelect;
      // Match the visual envelope used for the other select cards (~78
      // inner units tall). Width follows from the native aspect ratio.
      const innerScale = 78 / a.h;
      const dw = a.w * innerScale;
      ...
      const dx = -dw / 2;                       // center horizontally
      const dy = -25 - dh + bob / sc;           // bottom-anchor at foot line
      ctx.drawImage(zImg, dx, dy, dw, dh);
      ctx.restore();
      // Name label below the image
      ctx.font = `bold ${Math.max(10, s * 0.18)}px Arial`;
      ctx.fillStyle = char.accent;
  ...
  // Thad: render the ThadSelect.PNG photo as the card preview
  if (char.isThad) {
    const tImg = thadImages.ThadSelect;
    if (tImg) {
      const a = THAD_ANCHORS.ThadSelect;
      // Match the visual envelope used for Arnav's select card (~78
      // inner units tall). Width follows from the native aspect ratio.
      const innerScale = 78 / a.h;
      const dw = a.w * innerScale;
      const dh = a.h * innerScale;
      const dx = -dw / 2;                       // center horizontally
      const dy = -25 - dh + bob / sc;           // bottom-anchor at foot line
      ctx.drawImage(tImg, dx, dy, dw, dh);
      ctx.restore();
      // Name label below the image
      ctx.font = `bold ${Math.max(10, s * 0.18)}px Arial`;
      ctx.fillStyle = char.accent;
  ...
  // Arnav: render the ArnSelect.PNG photo as the card preview
  if (char.isArnav) {
    const aImg = arnavImages.ArnSelect;
    if (aImg) {
      const a = ARNAV_ANCHORS.ArnSelect;
      // Fit ~78 inner units tall (slightly taller than the procedural
      // fighter body since this photo is a torso/head shot, not a
      // full-body shot). Width follows from the native aspect ratio.
      const innerScale = 78 / a.h;
      const dw = a.w * innerScale;
      const dh = a.h * innerScale;
      const dx = -dw / 2;                       // center horizontally
...
function drawCharSelectScreen() {
  ...
  if (isRandom) {
    ...
    // Quote (italic, slightly smaller and darker than the description)
    if (current.quote) {
      ctx.font = `italic ${compact ? 10 : 13}px Arial`;
      ctx.fillStyle = '#777';
      ctx.fillText(current.quote, 480, infoBaseY + (compact ? 28 : 42));
    }
...
function drawBossSelectScreen() {
  ...
  // Grid layout (bosses + 1 for RANDOM)
  const totalItems = practiceBossList.length + 1;
  const total = totalItems;
  const perRow = Math.min(11, Math.ceil(total / 2)); // split roughly evenly into 2 rows
  ...
  for (let i = 0; i < total; i++) {
    const isRandom = i >= practiceBossList.length;
    const boss = isRandom ? null : practiceBossList[i];
    const row = Math.floor(i / perRow);
    const col = i % perRow;
    // Center each row
    ...
    const pulse = selected ? Math.sin(Date.now() * 0.004) * 0.5 + 0.5 : 0;
    const unlocked = isRandom || (defeatedBosses[boss.name] || false);

    // Card background
    const pad = 4;
    ctx.fillStyle = selected ? (isRandom ? 'rgba(255,215,0,0.15)' : 'rgba(255,40,40,0.15)') : 'rgba(255,255,255,0.03)';
    ctx.beginPath();
    ctx.roundRect(x - cardW / 2 + pad, y, cardW - pad * 2, cardH, 6);
    ctx.fill();
    ctx.strokeStyle = selected
      ? (isRandom ? `rgba(255,215,0,${0.6 + pulse * 0.4})` : `rgba(255,${100 + pulse * 155},0,${0.6 + pulse * 0.4})`)
      : unlocked ? '#444' : '#222';
    ...
    if (isRandom) {
      // RANDOM card
      const rPulse = selected ? Math.sin(Date.now() * 0.005) * 3 : 0;
      ctx.font = `bold ${24 + rPulse}px Arial`;
      ctx.textAlign = 'center';
      ctx.fillStyle = selected ? '#ffd700' : '#666';
      ctx.fillText('?', x, y + 42);
      ctx.font = 'bold 9px Arial';
      ctx.fillStyle = selected ? '#ffd700' : '#888';
      ctx.fillText('RANDOM', x, y + cardH - 6);
    } else if (unlocked) {
      // Draw boss portrait
    ...
    // Selection indicator
    if (selected) {
      ctx.strokeStyle = isRandom ? '#ffd700' : '#ff6b35';
      ctx.lineWidth = 2;
      ctx.beginPath();
  ...
  const isRandomSelected = bossSelectCursor >= practiceBossList.length;
  ctx.font = '16px Arial';
  ctx.textAlign = 'center';
  if (isRandomSelected) {
    ctx.fillStyle = '#ffd700';
    ctx.fillText('A random unlocked boss will be chosen', 480, descY);
  } else {
  ...
  // Lottery overlay
  if (lotteryActive && lotteryType === 'boss') {
    drawLotteryOverlay();
  }
...
function drawAssistSelectScreen() {
  ...
  for (let vi = 0; vi < visibleCount; vi++) {
    ...
    if (i < assists.length) {
      ...
      // Jayce: render the JacSelect photo instead of the procedural orb.
      if (assists[i].isJayce && jayceImages.JacSelect) {
        const img = jayceImages.JacSelect;
        const meta = JAYCE_ANCHORS.JacSelect;
        // Fit inside ~120w x 100h centered around y=-15. Tall portrait so
        // height tends to be the binding dimension.
        const sFit = Math.min(120 / meta.w, 100 / meta.h);
        const dw = meta.w * sFit;
        ...
        const sFit = Math.min(120 / meta.w, 90 / meta.h);
        const dw = meta.w * sFit;
        const dh = meta.h * sFit;
        const pulse = selected ? Math.sin(Date.now() * 0.005) * 3 : 0;
        ctx.drawImage(img, -dw / 2 - pulse / 2, -15 - dh / 2 - pulse / 2, dw + pulse, dh + pulse);
...
Fighter.prototype.callAssist = function(opponent) {
  ...
  } else if (this.assist.isKingRoller) {
    // Charge in from off-screen behind the player, headed toward the opponent.
    this.assistActive = {
      x: this.facing === 1 ? -140 : 1100,
      y: opponent.groundY,
      vx: this.facing * 10,
      timer: 150, hit: false,
      isKingRoller: true
    };
  } else if (this.assist.isJayce) {
    // Three-phase state machine: run on -> stop & fire -> exit.
    this.assistActive = {
      x: this.facing === 1 ? -100 : 1060,
      y: opponent.groundY,
      vx: this.facing * 7,
      facing: this.facing,
      phase: 'run',
      runUntilX: this.x + this.facing * 80,
      fireTimer: 14,            // initial pause once stopped before first shot
      fireInterval: 14,         // ~4 shots/sec cadence
      bulletsLeft: 3,
      bullets: [],
      timer: 240,               // safety despawn
      isJayce: true
    };
  } else {
    this.assistActive = {
...
Fighter.prototype.update = function(opponent, keys) {
  ...
  if (this.assistActive) {
    ...
    if (a.isWeedthorn) {
      ...
      // --- Jayce state machine: run -> fire -> exit ---
      if (a.phase === 'run') {
        a.x += a.vx;
        const reached = a.facing === 1 ? a.x >= a.runUntilX : a.x <= a.runUntilX;
        if (reached) {
          a.x = a.runUntilX;
          a.phase = 'fire';
          a.fireTimer = 14;
        ...
        a.fireTimer--;
        if (a.fireTimer <= 0 && a.bulletsLeft > 0) {
          // Spawn a bullet at the gun-muzzle position. Muzzle delta is
          // pre-computed in native pixels; scale to canvas, mirror x for facing.
          const m = JAYCE_ANCHORS.JacOnstage;
          const s = JAYCE_SCALE;
          const muzzleX = a.x + a.facing * (m.muzzleDX - m.anchorX) * s;
          const muzzleY = a.y + m.muzzleDY * s; // already negative (above feet)
          a.bullets.push({
            x: muzzleX,
            y: muzzleY,
            vx: a.facing * 16,
        ...
        if (a.bulletsLeft <= 0 && a.fireTimer <= -10) {
          a.phase = 'exit';
        }
      } else { // 'exit'
        a.x += a.vx;
      }
      // Update bullets (regardless of phase — they keep flying after Jayce leaves)
      for (let bi = a.bullets.length - 1; bi >= 0; bi--) {
        const b = a.bullets[bi];
        b.x += b.vx;
        b.life--;
        if (!b.hit && opponent.isHitAt(b.x, b.y, 22, 28)) {
          const diffMult = (!this.isPlayer && cpuDifficulty) ? cpuDifficulty.damageMult : 1;
          const dmg = this.assist.damage * this.char.stats.power * diffMult;
          opponent.takeDamage(
            dmg,
            { hitstun: 14, blockstun: 7, launch: false, knockbackForce: 2 },
            b.vx > 0 ? 1 : -1,
            false,
            { x: b.x, y: b.y }
          );
        ...
        if (b.hit || b.life <= 0 || b.x < -40 || b.x > 1000) {
          a.bullets.splice(bi, 1);
        }
      }
      // Despawn: jayce off-screen AND no bullets left in play.
      const jayceGone = a.phase === 'exit' && (a.x < -120 || a.x > 1080);
      if (a.timer <= 0 || (jayceGone && a.bullets.length === 0)) this.assistActive = null;
    } else if (a.isKingRoller) {
      a.x += a.vx;
      // Ram contact: hit roughly at mid-body height of the rolling backpack.
      if (!a.hit && opponent.isHitAt(a.x, a.y - 35, 60, 55)) {
        const diffMult = (!this.isPlayer && cpuDifficulty) ? cpuDifficulty.damageMult : 1;
        const dmg = this.assist.damage * this.char.stats.power * diffMult;
        opponent.takeDamage(
          dmg,
          { hitstun: 22, blockstun: 12, launch: false, knockbackForce: 7 },
          a.vx > 0 ? 1 : -1,
          false,
          { x: a.x, y: a.y - 35 }
        );
      ...
      // Despawn once the roller has rolled off the far side.
      if (a.timer <= 0 || a.x < -180 || a.x > 1140) this.assistActive = null;
    } else if (a.isSerpent) {
...
Fighter.prototype.handlePlayerInput = function(keys, opponent) {
  ...
  if (this.state !== 'attack') {
    ...
    if (this.isJay) {
      ...
      // Crouch + block mechanics removed — `down` now does nothing on the ground.
...
Fighter.prototype.handleAI = function(opponent) {
  ...
  if (this.aiTimer >= this.aiReactTime) {
    ...
    if (dist > 120) {
      ...
      // Block mechanic removed — fold former block-chance window into approach.
  ...
  switch (this.aiAction) {
    ...
    case 'attack':
      ...
      break;
    case 'block':
      // Block mechanic removed — no-op (kept for switch safety).
      break;
    case 'assist':
...
Fighter.prototype.drawArnav = function(ctx) {
  // Pick the appropriate sprite for the fighter's current state.
  let key = 'ArnStationary';
  if (this.state === 'attack' && this.currentAttack) {
    const n = this.currentAttack.name;
    if      (n === 'Jab')       key = 'ArnPunch';
    else if (n === 'Uppercut')  key = 'ArnUppercut';
    else if (n === 'Low Kick')  key = 'ArnLowKick';
    else if (n === 'High Kick') key = 'ArnHighKick';
    else                         key = 'ArnPunch'; // generic attack pose
  }
  const img = arnavImages[key] || arnavImages.ArnStationary;
  if (!img) return; // sprites not loaded yet — skip this frame
  const a = ARNAV_ANCHORS[key] || ARNAV_ANCHORS.ArnStationary;
  ...
  if (!skipBody) {
    // Sprites face right by default; mirror for facing = -1.
...
Fighter.prototype.drawThad = function(ctx) {
  // Pick the appropriate sprite for the fighter's current state.
  let key = 'ThadStationary';
  if (this.state === 'attack' && this.currentAttack) {
    const n = this.currentAttack.name;
    if      (n === 'Jab')       key = 'ThadPunch';
    else if (n === 'Uppercut')  key = 'ThadUppercut';
    else if (n === 'Low Kick')  key = 'ThadLowKick';
    else if (n === 'High Kick') key = 'ThadHighKick';
    else                         key = 'ThadPunch'; // any other attack
  }
  const img = thadImages[key] || thadImages.ThadStationary;
  if (!img) return; // sprites not loaded yet — skip this frame
  const a = THAD_ANCHORS[key] || THAD_ANCHORS.ThadStationary;
...
Fighter.prototype.drawZyllen = function(ctx) {
  ...
  if (this.state === 'attack' && this.currentAttack) {
    const n = this.currentAttack.name;
    if      (n === 'Jab')       key = 'ZylPunch';
    else if (n === 'Uppercut')  key = 'ZylUppercut';
    else if (n === 'Low Kick')  key = 'ZylLowKick';
    else if (n === 'High Kick') key = 'ZylHighKick';
    else                         key = 'ZylPunch'; // any other attack
  }
  const img = zyllenImages[key] || zyllenImages.ZylStationary;
  if (!img) return; // sprites not loaded yet — skip this frame
  const a = ZYLLEN_ANCHORS[key] || ZYLLEN_ANCHORS.ZylStationary;
...
Fighter.prototype.draw = function(ctx) {
  // Arnav uses sprite-based rendering — fully self-contained.
  if (this.char.isArnav && !(gameMode === 'practice' && !this.isPlayer && !this.char.isDrone)) {
    this.drawArnav(ctx);
    return;
  }
  // Thad uses sprite-based rendering — fully self-contained.
  if (this.char.isThad && !(gameMode === 'practice' && !this.isPlayer && !this.char.isDrone)) {
    this.drawThad(ctx);
    return;
  }
...
Fighter.prototype.drawAssistProjectile = function(a) {
  if (a.isWeedthorn) {
    ...
    // Draw bullets first (in world space, no transform inheritance from sprite).
    for (const b of a.bullets) {
      ctx.save();
      ctx.translate(b.x, b.y);
      // Bright orange capsule with a short motion-blur trail for readability.
      ctx.fillStyle = 'rgba(255,140,0,0.35)';
      ctx.beginPath();
      ctx.ellipse(-b.vx * 0.5, 0, 12, 3, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.ellipse(b.vx > 0 ? 2 : -2, 0, 2, 1.5, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
    }
    // Draw Jayce himself.
    const img = jayceImages.JacOnstage;
    if (img) {
      const meta = JAYCE_ANCHORS.JacOnstage;
      const s = JAYCE_SCALE;
      ctx.save();
      ctx.translate(a.x, a.y);
      // Subtle running-bob during 'run' / 'exit' phases.
      if (a.phase !== 'fire') {
        ctx.translate(0, Math.sin(Date.now() * 0.04) * 1.5);
      }
      // Native sprite faces RIGHT; mirror when facing left.
    ...
    // Note: outer drawAssistProjectile wraps each branch in a save/restore,
    // so we must NOT leave any persistent transform — but we did the inner
    // saves above and the outer caller restores afterward, so we're safe.
    // We have to translate to (0,0) at the end because the outer caller
    // doesn't expect us to restore the outer save itself.
  } else if (a.isKingRoller) {
    const img = kingRollerImages.RollJoust;
    if (img) {
      const meta = KING_ROLLER_ANCHORS.RollJoust;
      const s = KING_ROLLER_SCALE;
      ctx.translate(a.x, a.y);
      // Native joust pose leans LEFT (handle pokes upper-left, wheels lower-right).
      // Mirror horizontally when the roller is charging right.
      if (a.vx > 0) ctx.scale(-1, 1);
      // Subtle bob to suggest momentum on the wheels.
...
artikIconImg.src = 'Assets/Icons/artikIconImg.png';
